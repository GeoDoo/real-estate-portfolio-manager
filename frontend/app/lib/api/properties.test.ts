...
describe("propertiesAPI", () => {
  ...
  it("getAll fetches all properties", async () => {
    mockApiRequest.mockResolvedValue({
      items: [{ id: "1", address: "A", postcode: "P1" }],
    });
  ...
  it("create posts new property", async () => {
    mockApiRequest.mockResolvedValue({
      data: { id: "2", address: "B", postcode: "P2" },
    });
    const { propertiesAPI } = await import("./properties");
  ...
  it("update puts property", async () => {
    mockApiRequest.mockResolvedValue({
      data: { id: "3", address: "C", postcode: "P3" },
    });
    const { propertiesAPI } = await import("./properties");
    const result = await propertiesAPI.update("3", {
      address: "C",
      postcode: "P3",
    });
    expect(mockApiRequest).toHaveBeenCalledWith(
      "http://localhost:5050/api/properties/3",
  ...
  it("getById fetches property by id", async () => {
    mockApiRequest.mockResolvedValue({
      data: { id: "4", address: "D", postcode: "P4" },
    });
    const { propertiesAPI } = await import("./properties");
    const result = await propertiesAPI.getById("4");
    expect(mockApiRequest).toHaveBeenCalledWith(
      "http://localhost:5050/api/properties/4",
    );
  ...
  it("delete calls delete endpoint", async () => {
    ...
    expect(mockApiRequest).toHaveBeenCalledWith(
      "http://localhost:5050/api/properties/5",
  ...
  it("assignToPortfolio patches property", async () => {
    ...
    expect(mockApiRequest).toHaveBeenCalledWith(
      "http://localhost:5050/api/properties/6",
  ...
  it("handles errors from apiRequest", async () => {
    ...
    await expect(
      propertiesAPI.create({ address: "X", postcode: "P1" }),
    ).rejects.toThrow("fail");
    await expect(
      propertiesAPI.update("id", { address: "Y", postcode: "P2" }),
    ).rejects.toThrow("fail");
    await expect(propertiesAPI.getById("id")).rejects.toThrow("fail");
    await expect(propertiesAPI.delete("id")).rejects.toThrow("fail");
    await expect(propertiesAPI.assignToPortfolio("id", null)).rejects.toThrow(
      "fail",
    );
  });
});
