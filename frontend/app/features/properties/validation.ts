...
function isIndexable(
  obj: unknown,
): obj is Record<string, number | undefined | null> {
  return typeof obj === "object" && obj !== null;
}

export function hasValidValuation(
  valuation:
  ...
  if (
    !requiredFields.every(
      (f) => typeof valuation[f] === "number" && valuation[f]! > 0,
    )
  )
    return false;
  if (
    !optionalFields.every(
      (f) =>
        valuation[f] === undefined ||
        valuation[f] === null ||
        (typeof valuation[f] === "number" && valuation[f]! >= 0),
    )
  )
    return false;
  return true;
...
export function isValidValuationForm(
